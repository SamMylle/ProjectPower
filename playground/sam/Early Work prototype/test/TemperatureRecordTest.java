...
public class TemperatureRecordTest {
	...
	TemperatureRecord record = new TemperatureRecord(5);
	@Before
	public void setUp() throws Exception {
		record = new TemperatureRecord(5);
	}
	...
	public void testRecord() {
		...
		for (int i = 0; i < 100; i++){
			record.addValue(i);
			LinkedList<Double> list = new LinkedList<Double> (record.getRecord());
	...
	public void testConstr() {
		for (int i = 1; i < 20; i++){
			TemperatureRecord newRecord = new TemperatureRecord(i);
			for (int j = 0; j < 2*i; j++){
